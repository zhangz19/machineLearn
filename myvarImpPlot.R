myvarImpPlot <- function (x, sort = TRUE, n.var = min(30, nrow(x$importance)), 
                          ...
                          ...) 
{
  ...
  row.names(imp) <- XnamP
  nmeas <- 1 #if only do %IncMSE
  
  
  if (nmeas > 1) {
    op <- par(mfrow = c(1, 2), mar = c(3, 4, 1, 1), mgp = c(1.5, 0.8, 0), oma = c(0, 0, 2, 0), no.readonly = TRUE, cex.lab=.9, cex.axis=.9)
    on.exit(par(op))
  }
  for (i in 1:nmeas) {
    ord <- if (sort) 
