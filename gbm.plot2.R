...
gbm.plot2 <- function (gbm.object, variable.no = 0, smooth = FALSE, rug = TRUE, 
                       ...
                       y.label = "fitted fun", x.label = NULL, show.contrib = TRUE, 
                       plot.layout = c(3, 4), ...) 
{
  ...
  for (j in c(1:n.plots)) {
    ...
    if (n.plots == 1) {
      ...
      if (show.contrib) {
        x.label <- paste(var.name[k], "  (", round(gbm.object$contributions[k, 
                                                                            2], 1), "%)", sep = "")
      }
    }
    else {
      k <- match(gbm.object$contributions$var[j], pred.names)
      if (show.contrib) {
        x.label <- paste(var.name[k], "  (", round(gbm.object$contributions[j, 
                                                                            2], 1), "%)", sep = "")
      }
    ...
    if (common.scale) {
      plot(predictors[[j]], responses[[j]], ylim = c(ymin, 
                                                     ymax), type = "n", xlab = x.label, ylab = y.label,
           ...)
      grid(col='gray40')
      lines(predictors[[j]], responses[[j]], lwd=2, col='blue2')
    }
    else {
